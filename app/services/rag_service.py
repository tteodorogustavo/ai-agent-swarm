...
# --- 2. RAG Part 1: Ingestion Logic (The "Writer") ---
...
def get_vector_store(embeddings: OpenAIEmbeddings) -> FAISS:
    ...
    if os.path.exists(faiss_index_file):
        ...
        logging.info(f"No existing index found at {faiss_index_file}. Creating new vector store.")
...
def run_full_ingestion():
    ...
    try:
        ...
        logging.error(f"Failed to initialize embeddings or vector store: {e}", exc_info=True)
        return
    ...
    logging.info(f"Indexing complete. Added: {result.get('num_added', 0)}, Updated: {result.get('num_updated', 0)}, Deleted: {result.get('num_deleted', 0)}")
...
def create_rag_retriever() -> BaseRetriever:
    """
    Creates the "Librarian" (RAG Retriever) that the Knowledge Agent will use.
    ...
    if not os.path.exists(index_path):
        raise FileNotFoundError("Vector store not found: {}".format(index_path))
