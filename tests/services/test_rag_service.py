...
@patch("app.services.rag_service.index")
@patch("app.services.rag_service.SQLRecordManager")
@patch("app.services.rag_service.get_vector_store")
@patch("app.services.rag_service.load_and_split_documents")
...
def test_run_full_ingestion(
    ...
    mock_vector_store_instance.save_local.assert_called_once_with(rag_service.VECTOR_STORE_PATH)
...
def test_create_rag_retriever(mock_path_exists, MockEmbeddings, MockFAISS):
    ...
    mock_path_exists.assert_called_with(os.path.join(rag_service.VECTOR_STORE_PATH, "index.faiss"))
...
@patch("app.services.rag_service.os.path.exists", return_value=False)
